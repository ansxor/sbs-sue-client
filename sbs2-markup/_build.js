...
code: function(args, contents) {
	...
	node.appendChild(Parse.options.Highlight.highlight(contents, language))
	return {node:node}
},
// inline code
...
audio: function(args, contents) {
	var url = args[""]
	url = Parse.options.filterURL(url, 'audio')
	if (url == null)
...
video: function(args, contents) {
	var url = args[""]
	url = Parse.options.filterURL(url, 'video')
	if (url == null)
		return link(args, url)
...
youtube: function(args, contents, preview) { //todo: use contents?
	var url = args[""]
	url = Parse.options.filterURL(url, 'youtube')
	if (url == null)
		return link(args, contents)
	
	var match = Parse.options.getYoutubeID(url)
	...
	if (match) {
		...
		if (!preview)
			Parse.options.getYoutube(match, function(data) {
...
image: function(args, alt) {
	var url = args[""]
	url = Parse.options.filterURL(url, 'image')
	if (url == null)
		return Parse.options.link(args, url)
